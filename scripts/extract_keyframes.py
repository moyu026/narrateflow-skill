...
from _config import load_keyframe_config
...
def main() -> None:
    parser = argparse.ArgumentParser(description="从视频中抽取画面变化关键帧")
    parser.add_argument("--video", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--config")
    parser.add_argument("--fps-sample", type=float)
    ...
    settings = load_keyframe_config(Path(args.config) if args.config else None)
